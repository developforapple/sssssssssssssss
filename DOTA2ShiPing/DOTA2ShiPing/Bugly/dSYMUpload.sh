...
#
#
# BUGLY_APP_ID、BUGLY_APP_KEY、BUGLY_APP_BUNDLE_ID
# 已在 Build Setting User-Defined Setting中进行了配置

#BUGLY_APP_ID=""
#BUGLY_APP_KEY=""
#BUGLY_APP_BUNDLE_ID=""

echo 'BuglyAppID : '  ${BUGLY_APP_ID}
echo 'BuglyAppKey : ' ${BUGLY_APP_KEY}
echo 'BundleID : ' + ${BUGLY_APP_BUNDLE_ID}
echo 'BuglyJarPath : ' ${BUGLY_JAR_PATH}
...
# jar工具的路径在 BuildSetting/UserDefined 设置
BUGLY_SYMBOL_JAR_PATH=${BUGLY_JAR_PATH}
# 查找添加到系统目录的jar工具
...
function dSYMUpload() {
    ...
    echo "-----------------------------"
    STATUS=$(/usr/bin/curl -k "${DSYM_UPLOAD_URL}" --form "api_version=1" --form "app_id=${P_APP_ID}" --form "app_key=${P_APP_KEY}" --form "symbolType=2"  --form "bundleId=${BUGLY_APP_BUNDLE_ID}" --form "productVersion=${BUGLY_APP_VERSION}" --form "fileName=${P_BSYMBOL_ZIP_FILE_NAME}" --form "file=@${P_BSYMBOL_ZIP_FILE}" --verbose)
    echo "-----------------------------"
    ...
    if [ ! "${STATUS}" ]; then
        ...
        echo "Success to upload the dSYM for the app [${BUGLY_APP_BUNDLE_ID} ${BUGLY_APP_VERSION}]"
...
function runInXcode(){
    ...
    echo "Bundle Identifier: ${BUGLY_APP_BUNDLE_ID}"
    ...
    #
    run ${BUGLY_APP_ID} ${BUGLY_APP_KEY} ${BUGLY_APP_BUNDLE_ID} ${BUGLY_APP_VERSION} ${DWARF_DSYM_FOLDER_PATH} ${BUILD_DIR}/BuglySymbolTemp ${UPLOAD_DSYM_ONLY}
}

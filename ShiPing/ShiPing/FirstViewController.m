...
//    return;
    ...
    // 跳过第二步，使用工程目录中的items_game.txt文件 进行第三步
//    NSLog(@"跳过 step2");
//    [self test];
//    return;
    
    // 第二步：从steam服务器下载饰品数据
    NSLog(@"step2: 获取 schemaURL ");
    ...
    NSLog(@"items_game_url: %@",items_game_url);
    
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:items_game_url]];
...
- (void)test
{
    NSString *items_game_url = [[NSBundle mainBundle] pathForResource:@"items_game" ofType:@"txt"];
    VDFNode *node = [VDFParser parse:[NSData dataWithContentsOfFile:items_game_url]];
    [self parseItemsGameData:node];
}

- (void)parseItemsGameData:(VDFNode *)node
{
    NSLog(@"step3：创建数据model");
    ...
    
    {
        ...
        NSLog(@"上传主语言文件 开始");
        {
            ...
            [file saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                ...
                NSLog(@"上传主语言文件 完成");
                NSLog(@"更新主文件版本号为：%lld",curLangVersion);
                ...
                NSLog(@"上传主语言文件：%d%%",(int)percentDone);
            }];
        }
    }
}
                   ...
                   completion:(void(^)(ServiceType type))completion
{
    ...
    
    {
        ...
        NSLog(@"服务器语言补丁版本： %lld",langPatchVersion);
    ...
    
    {
        ...
        NSLog(@"新的语言补丁版本为:%lld",curLangPatchVersion);
        ...
        NSLog(@"上传语言补丁文件 开始");
        {
            ...
            [file saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                ...
                NSLog(@"上传语言补丁 完成");
                NSLog(@"更新语言补丁版本号为：%lld",curLangPatchVersion);
